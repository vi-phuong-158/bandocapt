(function (root, factory) {
    ...
    // Returns the canonical SITE_TYPES code (never a label), or null when `value` is empty or not
    // a readable site type. Mirrors `toCanonicalServices`: callers that need to branch on place
    // type (map filtering, marker classification) should key off this, not off raw sheet text.
    function toCanonicalSiteType(value) {
        const raw = code(value);
        if (!raw || !isReadableSiteType(raw)) return null;
        return LEGACY_SITE_TYPES[raw] || raw;
    }
    ...
    return Object.freeze({ SITE_TYPES, SERVICES, REQUEST_TYPES, LEGACY_SITE_TYPES, LEGACY_SERVICES, LEGACY_SERVICE_FALLBACKS, LEGACY_SERVICE_LABELS, requestKind, requestType, isWritableSiteType, isAcceptedWriteSiteType, isReadableSiteType, isWritableService, isAcceptedWriteService, isReadableService, normalizeServices, toCanonicalServices, toCanonicalSiteType, displaySiteType, displayService, generateDisplayName, sanitizeDisplayNameOverride, locationName, formDefinition });
});
