...
const { callGateway, MUTATION_TIMEOUT_MS, MUTATION_MAX_ATTEMPTS } = require('./staff-gateway-client');
const { verifyGoogleIdToken } = require('./staff-auth');
const { resolveMapsCoordinates } = require('./staff-maps-resolver');
const {
...
const REQUEST_TEXT_LIMITS = Object.freeze({
    operationId: 120, requestType: 80, unitCode: 160, targetRecordId: 160, recordId: 160, snapshotHash: 64, eventType: 40,
    locationName: 200, siteType: 80, address: 500, publicPhone: 80, mapsUrl: 2000,
    coordinates: 200, cccdServiceMode: 80, serviceSchedule: 1000, servedUnits: 1000,
    searchAliases: 1000, submitterName: 200, submitterPhone: 80, reviewNote: 2000,
    note: 2000,
});
const REQUEST_ARRAY_LIMITS = Object.freeze({ services: { items: 16, itemLength: 80 } });
...
function normalizeText(value, maxLength) {
    ...
    if (typeof value !== 'string') throw staffError('STAFF_REQUEST_INVALID');
    const normalized = value.trim();
    if (normalized.length > maxLength) throw staffError('STAFF_REQUEST_INVALID');
    return normalized;
}

function normalizeTextArray(value, { items, itemLength }) {
    if (value === undefined) return undefined;
    if (!Array.isArray(value) || value.length > items) throw staffError('STAFF_REQUEST_INVALID');
    return value.map(item => normalizeText(item, itemLength)).filter(Boolean);
}

function normalizeRequestBody(body) {
    if (!body || typeof body !== 'object' || Array.isArray(body)) throw staffError('INVALID_BODY');
    const normalized = { ...body };
    Object.entries(REQUEST_TEXT_LIMITS).forEach(([name, maxLength]) => {
        if (Object.prototype.hasOwnProperty.call(body, name)) normalized[name] = normalizeText(body[name], maxLength);
    });
    Object.entries(REQUEST_ARRAY_LIMITS).forEach(([name, limits]) => {
        if (Object.prototype.hasOwnProperty.call(body, name)) normalized[name] = normalizeTextArray(body[name], limits);
    });
    return normalized;
}
...
function buildRequestPayload(body, session, unit, requestId) {
    const image = validateStaffImage(body.image);
    const fields = {
        ...
        // Verified Google display name is authoritative when present; the client cannot override it.
        // A client-submitted value is only used as a fallback while no verified name is available.
        submitter_name: session.name || body.submitterName,
...
const STAFF_REQUEST_TYPES = new Set(['Thêm địa điểm mới', 'Cập nhật địa điểm đang có', 'Báo địa chỉ hoặc vị trí sai', 'Báo địa điểm ngừng hoạt động']);

function toStaffPendingRequest(value, units) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
    const unitCode = String(value.unitCode || '').trim();
    const type = String(value.type || '').trim();
    const status = String(value.status || '').trim();
    if (!unitCode || !STAFF_REQUEST_TYPES.has(type) || status !== 'PENDING' || !findAuthorizedUnit(units, unitCode)) return null;
    return {
        locationId: String(value.locationId || '').trim(),
        unitCode,
        type,
        status,
        submittedAt: String(value.submittedAt || '').trim(),
    };
}
...
function createStaffApi(overrides = {}) {
    const deps = {
        ...
        resolveMaps: overrides.resolveMaps || resolveMapsCoordinates,
    };
    ...
    async function config(req, res) {
        setNoStore(res);
        if (req.method !== 'GET') return res.status(405).json({ ok: false, error: { code: 'METHOD_NOT_ALLOWED' } });
        if (!deps.env.GOOGLE_CLIENT_ID) return sendStaffError(res, staffError('STAFF_AUTH_CONFIG_INVALID'));
        return sendStaffData(res, { googleClientId: String(deps.env.GOOGLE_CLIENT_ID) });
    }
    ...
    async function google(req, res) {
        ...
        try {
            ...
            const token = createStaffSession({ sub: identity.sub, email: identity.email, name: identity.name, now: deps.now() }, deps.env.STAFF_SESSION_SECRET);
            setCookie(res, serializeStaffSessionCookie(token));
            return sendStaffData(res, { user: { email: identity.email, name: identity.name }, units });
    ...
    async function session(req, res) {
        ...
        try {
            ...
            return sendStaffData(res, { user: { email: current.email, name: current.name }, units });
        } catch (error) {
    ...
    async function locations(req, res) {
        ...
        try {
            if (req.method !== 'GET') throw staffError('METHOD_NOT_ALLOWED');
            const { session, units } = await requireProtectedRequest(req, deps);
            const allowed = new Set(units.map(unit => unit.unitCode.toLowerCase()));
            const [data, requestResult] = await Promise.all([
                getCachedPublicLocations(deps),
                deps.gatewayCall('listStaffRequestStatuses', { email: session.email }, { env: deps.env }),
            ]);
            ...
            const pendingRequests = (Array.isArray(requestResult?.requests) ? requestResult.requests : [])
                .map(value => toStaffPendingRequest(value, units)).filter(Boolean);
            return sendStaffData(res, { locations, pendingRequests });
        } catch (error) {
            if (error.code === 'STAFF_ACCESS_REVOKED') setCookie(res, clearStaffSessionCookie());
            return sendStaffError(res, error);
        }
    ...
    async function mapsResolve(req, res) {
        setNoStore(res);
        try {
            if (req.method !== 'POST') throw staffError('METHOD_NOT_ALLOWED');
            assertMutationTrust(req, deps.env);
            getSession(req, deps.env);
            const body = parseRequestBody(req);
            const mapsUrl = typeof body.mapsUrl === 'string' ? body.mapsUrl.trim() : '';
            if (!mapsUrl || mapsUrl.length > REQUEST_TEXT_LIMITS.mapsUrl) throw staffError('COORDINATE_INVALID_LINK');
            const coordinates = await deps.resolveMaps(mapsUrl);
            return sendStaffData(res, { coordinates });
        } catch (error) { return sendStaffError(res, error); }
    }
    ...
    async function verification(req, res) {
        ...
        try {
            ...
            const body = normalizeRequestBody(parseRequestBody(req));
            ...
            }, { env: deps.env, requestId, timeoutMs: MUTATION_TIMEOUT_MS, maxAttempts: MUTATION_MAX_ATTEMPTS });
    ...
    async function requests(req, res) {
        ...
        try {
            ...
            if (TARGET_REQUEST_TYPES.has(requestType)) {
                ...
                if (requestType === 'Cập nhật địa điểm đang có' || requestType === 'Báo địa chỉ hoặc vị trí sai') {
                    const targetServices = Array.isArray(currentTarget.services) ? currentTarget.services : (currentTarget.services ? String(currentTarget.services).split('|') : []);
                    const effectiveServices = (body.services && body.services.length) ? body.services : targetServices;
                    const effectiveSiteType = body.siteType !== undefined && body.siteType !== '' ? body.siteType : (currentTarget.siteType || currentTarget.site_type || 'HEADQUARTERS');
                    const effectiveLocationName = body.locationName !== undefined && body.locationName !== '' ? body.locationName : currentTarget.name;
                    const effectiveAddress = body.address !== undefined && body.address !== '' ? body.address : (currentTarget.address || '');
                    const effectivePhone = body.publicPhone !== undefined && body.publicPhone !== '' ? body.publicPhone : (currentTarget.phone || currentTarget.public_phone || '');
                    const targetMaps = currentTarget.sourceGoogleMapsUrl ?? currentTarget.google_maps_url ?? currentTarget.googleMapsUrl ?? '';
                    const effectiveCccd = body.cccdServiceMode !== undefined && body.cccdServiceMode !== '' ? body.cccdServiceMode : (currentTarget.cccdServiceMode || currentTarget.cccd_service_mode || 'NOT_PROVIDED');
                    const effectiveSchedule = body.serviceSchedule !== undefined && body.serviceSchedule !== '' ? body.serviceSchedule : (currentTarget.serviceSchedule || currentTarget.service_schedule || '');
                    const effectiveServedUnits = body.servedUnits !== undefined && body.servedUnits !== '' ? body.servedUnits : (currentTarget.servedUnits || currentTarget.served_units || '');
                    const effectiveAliases = body.searchAliases !== undefined && body.searchAliases !== '' ? body.searchAliases : (currentTarget.searchAliases || currentTarget.search_aliases || '');

                    const servicesChanged = effectiveServices.slice().sort().join('|') !== targetServices.slice().sort().join('|');
                    const siteTypeChanged = effectiveSiteType !== (currentTarget.siteType || currentTarget.site_type || 'HEADQUARTERS');
                    const nameChanged = effectiveLocationName !== currentTarget.name;
                    const addressChanged = effectiveAddress !== (currentTarget.address || '');
                    const phoneChanged = effectivePhone !== (currentTarget.phone || currentTarget.public_phone || '');
                    const mapsChanged = Boolean(body.mapsUrl && body.mapsUrl !== targetMaps);
                    const coordinatesChanged = Boolean(body.coordinates && body.coordinates !== (currentTarget.coordinates || ''));
                    const cccdChanged = effectiveCccd !== (currentTarget.cccdServiceMode || currentTarget.cccd_service_mode || 'NOT_PROVIDED');
                    const scheduleChanged = effectiveSchedule !== (currentTarget.serviceSchedule || currentTarget.service_schedule || '');
                    const servedUnitsChanged = effectiveServedUnits !== (currentTarget.servedUnits || currentTarget.served_units || '');
                    const aliasesChanged = effectiveAliases !== (currentTarget.searchAliases || currentTarget.search_aliases || '');
                    const imageChanged = Boolean(body.image);

                    if (!servicesChanged && !siteTypeChanged && !nameChanged && !addressChanged && !phoneChanged && !mapsChanged && !coordinatesChanged && !cccdChanged && !scheduleChanged && !servedUnitsChanged && !aliasesChanged && !imageChanged) {
                        throw staffError('NO_CHANGES');
                    }
                }
            }
            ...
            if (requestType === 'Thêm địa điểm mới' && !payload.image) throw staffError('IMAGE_REQUIRED');
            try {
                return sendStaffData(res, await deps.gatewayCall(action, payload, {
                    env: deps.env, requestId, timeoutMs: MUTATION_TIMEOUT_MS, maxAttempts: MUTATION_MAX_ATTEMPTS,
                }));
    ...
    return { config, csrf, google, logout, session, locations, requests, verification, mapsResolve };
}
