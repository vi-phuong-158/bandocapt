...
function filterGovernedMatches(matches = [], query = '', now = new Date()) {
    ...
    // Vai trò nguồn + hiệu lực là ràng buộc CỨNG (fail-closed). Cap là ưu tiên MỀM.
    const governed = (matches || []).filter(match => {
        const metadata = match?.metadata || {};
        const policy = sourcePolicy(metadata);
        return !!policy && hasApprovedSourceRole(metadata) && isWithinValidity(metadata, now);
    });
    if (!cap) return governed;
    const sameCap = governed.filter(match => {
        const metadata = match?.metadata || {};
        return !sourcePolicy(metadata).constrainedByCap
            || normalizeCap(metadata.cap_normalized || metadata.cap) === cap;
    });
    // T3.6: nếu KHÔNG có thủ tục nào đúng cấp người dùng nêu (vd hỏi "đăng ký xe cấp
    // xã" nhưng corpus chỉ có bản cấp tỉnh), không loại sạch để bot phải từ chối oàn —
    // trả về nhóm governed cấp khác để bot vẫn nêu được thủ tục ở cấp thực tế.
    return sameCap.length ? sameCap : governed;
}

function buildGovernanceFilter(categoryClauses = [], cap = '', sourceTypes = Object.keys(SOURCE_POLICIES)) {
    const allowedTypes = new Set(sourceTypes);
    const sourceClauses = Object.entries(SOURCE_POLICIES).filter(([sourceType]) => allowedTypes.has(sourceType)).map(([sourceType, policy]) => {
        const clauses = [
...
function buildCurrentProcedureFilter(categoryClauses = [], cap = '') {
    return buildGovernanceFilter(categoryClauses, cap, ['tthc']);
}
