...
// Ký tự cuối KHÔNG được là dấu câu: URL đứng cuối câu ("...nộp tại https://x.gov.vn.") mà regex
// ăn luôn dấu chấm thì khi redact sẽ mất luôn dấu kết câu, để lại mệnh đề cụt không có dấu chấm.
const MAPS_URL_PATTERN = /https?:\/\/(?:www\.)?(?:google\.[a-z.]+\/maps|maps\.google\.[a-z.]+)\/[^\s)\]>]*[^\s)\]>.,;:!?]/gi;
const PUBLIC_URL_PATTERN = /https?:\/\/[^\s<>\])}]*[^\s<>\])}.,;:!?]/gi;
...
const DURATION_PATTERN = /(?<![\p{L}\p{N}_])\d+\s*(?:giờ|hours?|小时|시간|ngày làm việc|working days?|工作日|영업일)(?![\p{L}\p{N}_])/giu;
...
// Ranh giới câu hoàn chỉnh: dấu kết câu (kèm dấu đóng ngoặc/nháy/markdown nếu có) theo sau bởi
// khoảng trắng hoặc cuối chuỗi. Yêu cầu khoảng trắng phía sau để không nhận nhầm dấu chấm trong
// URL/số thập phân (vd "dichvucong.gov.vn", "21.304528") làm điểm cắt.
...
const TRUNCATION_NOTICES = {
    ...
    en: '\n\n*(The answer was shortened due to the length limit — ask a follow-up and I will continue.)*',
    zh: '\n\n*（由于长度限制，回答已被截短——请继续提问，我会补充其余内容。）*',
    ko: '\n\n*(길이 제한으로 답변이 축약되었습니다 — 이어서 질문하시면 나머지를 안내해 드리겠습니다.)*',
};

function getTruncationNotice(userLang) {
    return TRUNCATION_NOTICES[userLang] || TRUNCATION_NOTICES.en;
}

// Khi model bị cắt vì MAX_TOKENS, văn bản thường đứt giữa câu. Lùi về ranh giới an toàn gần nhất:
// dấu kết câu có khoảng trắng phía sau, hoặc hết một dòng trọn vẹn (bullet/heading). Nếu không tìm
// được ranh giới nào thì bỏ fragment; giữ lại sẽ tiếp tục hiển thị đúng lỗi đứt giữa câu cần ngăn.
function trimToSentenceBoundary(text) {
    ...
    let match;
    while ((match = SENTENCE_BOUNDARY_PATTERN.exec(value)) !== null) {
        lastBoundary = match.index + match[0].length;
    }
    const lastNewline = value.lastIndexOf('\n');
    const cut = Math.max(lastBoundary, lastNewline);
    if (cut <= 0) return '';
    return value.slice(0, cut).trimEnd();
}

// T2B-1: chỉ phát ra phần đã kết thúc ở ranh giới câu hoặc dòng/bullet. Phần còn lại
// giữ trong buffer để một claim (đặc biệt số điện thoại/phí/thời hạn) không lộ ra UI
// trước khi validate. `flush` chỉ dùng khi stream đã kết thúc.
function takeCompleteSegment(text, { flush = false } = {}) {
    const value = String(text || '');
    if (!value) return { segment: '', remainder: '' };
    if (flush) return { segment: value, remainder: '' };
    ...
    const lastNewline = value.lastIndexOf('\n');
    if (lastNewline >= 0) cut = Math.max(cut, lastNewline + 1);
    return { segment: value.slice(0, cut), remainder: value.slice(cut) };
}
...
function validateAnswer(text, allowed = {}) {
    ...
    const allowedUrls = new Set([
        ...Array.from(allowed.urls || [], normalizeUrl),
        ...Array.from(allowed.mapsUrls || [], normalizeUrl),
        ...Array.from(String(allowed.legalCorpus || '').match(PUBLIC_URL_PATTERN) || [], normalizeUrl),
    ]);
    ...
    const redact = (pattern, type, isAllowed, replacement) => {
        sanitizedText = sanitizedText.replace(pattern, value => {
            ...
            violations.push({ tier: type === 'phone' || type === 'maps_url' || type === 'url' || type === 'coords' ? 1 : 2, type, value, action: 'redact' });
            return replacement;
        });
    };

    redact(MAPS_URL_PATTERN, 'maps_url', value => mapsUrls.has(normalizeUrl(value)), '');
    // URL hành chính trong câu trả lời phải xuất hiện nguyên văn trong tài liệu/trụ sở đã xác minh.
    // Chặn cả domain giả gần giống (vd xuatnhhapcanh) thay vì chỉ dựa vào model gõ đúng.
    redact(PUBLIC_URL_PATTERN, 'url', value => allowedUrls.has(normalizeUrl(value)), '');
