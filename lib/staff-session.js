...
function createStaffSession({ sub, email, now = Date.now(), ttlSeconds = SESSION_TTL_SECONDS }, secret) {
    ...
    const payload = base64url(JSON.stringify({ v: 1, sub: normalizedSub, email: normalizedEmail, iat, exp }));
...
function verifyStaffSession(token, secret, now = Date.now()) {
    ...
    return Object.freeze({ v: 1, sub: payload.sub, email: payload.email.toLowerCase(), iat: payload.iat, exp: payload.exp });
}
