...
const PREPOSITION_LOCATION_PATTERN = /\b(?:ở|tai|khu vuc|khu vực|dia ban|địa bàn)\s+(?!đâu\b|dau\b|chỗ nào\b|cho nao\b|đâu đây\b|dau day\b|may gio\b|mấy giờ\b|lam\b|làm\b|sao\b|gi\b|nao\b)[a-z0-9à-ỹ]+/i;
const PREPOSITION_LOCATION_NORM_PATTERN = /\b(?:o|tai|khu vuc|dia ban)\s+(?!dau\b|cho nao\b|dau day\b|may gio\b|lam\b|lam sao\b|gi\b|nao\b)[a-z0-9]+/i;
const ENGLISH_PREPOSITION_PATTERN = /\b(?:in|at)\s+[a-z0-9\s]+(?:ward|commune|district|province|city)\b|\b(?:in|at)\s+[A-Z][a-z]+/i;
...
function extractLocationEvidence(currentMessage, history = []) {
    ...
    if (looksLikeResidenceDeclaration(raw)) {
        return { hasLocationEvidence: true, source: 'residence_declaration', text: raw };
    }
    ...
    if (PREPOSITION_LOCATION_PATTERN.test(raw) || PREPOSITION_LOCATION_NORM_PATTERN.test(normalized) ||
        ENGLISH_PREPOSITION_PATTERN.test(raw)) {
        return { hasLocationEvidence: true, source: 'preposition_location', text: raw };
    }
...
function hasLocationEvidence(currentMessage, history = []) {
    return extractLocationEvidence(currentMessage, history).hasLocationEvidence;
}

function isLocationLookupRequested(currentMessage, history) {
    // A service keyword (for example "căn cước") can request the location branch, but it
    // is not location evidence. History is intentionally excluded from this decision except
    // for the immediately preceding assistant question with an explicit location contract.
    const haystack = normalizeLabel(currentMessage);
    return LOCATION_TRIGGER_PATTERNS.some(pattern => pattern.test(haystack)) ||
        looksLikeResidenceDeclaration(currentMessage) ||
        (looksLikeShortLocationText(currentMessage) && !wasAssistantAskingForNationality(history)) ||
        wasAssistantAskingForLocation(history);
}

function buildLookupTexts(currentMessage, history, hasEvidence) {
    const texts = [];
    const add = (text, source, allowBareName, allowApprovedAlias = false, allowRegionStopwords = false) => {
        const normalized = normalizeLabel(text);
        ...
        if (!texts.some(item =>
            ...
            texts.push({ normalized, source, allowBareName, allowApprovedAlias, allowRegionStopwords });
        }
    };
    ...
    if (evidenceResolved) {
        add(currentMessage, 'current-loose', true, true, wasAssistantAskingForLocation(history));
    }
...
function scoreLocationMatch(location, lookupTexts) {
    ...
    for (const lookupText of lookupTexts) {
        ...
        if (lookupText.allowBareName && bareName &&
            ...
            aliasAppearsInText(bareName, lookupText.normalized)) {
...
function findVerifiedLocationMatches(currentMessage, history, dataset) {
    ...
    const evidence = extractLocationEvidence(currentMessage, history);
