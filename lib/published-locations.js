...
function findVerifiedLocationMatches(currentMessage, history, dataset) {
    ...
    if (matchedConflicts.length > 0) {
        return { lookupRequested: true, status: 'ambiguous_conflict', matches: [], conflicts: matchedConflicts, lookupTexts };
    }
    if (matchedLocations.length === 0) {
        return { lookupRequested: true, status: 'no_match', matches: [], lookupTexts };
    }
    ...
    if (topMatches.length > 1) {
        return { lookupRequested: true, status: 'ambiguous_match', matches: topMatches, lookupTexts };
    }

    return { lookupRequested: true, status: 'matched', matches: [matchedLocations[0]], lookupTexts };
}
