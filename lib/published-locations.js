...
const { resolvePublicSpreadsheetId } = require('./location-workbook-config');
...
async function fetchGoogleVisualizationPayload(options = {}) {
    const sheetId = options.sheetId || resolvePublicSpreadsheetId();
...
function scoreLocationMatch(location, lookupTexts) {
    ...
    for (const lookupText of lookupTexts) {
        ...
        if (lookupText.allowBareName && bareName && !REGION_STOPWORDS.has(bareName) && aliasAppearsInText(bareName, lookupText.normalized)) {
...
function asksForCitizenIdLocation(message, history) {
    const text = [message, ...getRecentText(history, 'user', 2)].map(normalizeLabel).join(' ');
    return /\b(can cuoc|cccd)\b/.test(text);
}
