...
function detectLanguage(text) {
    ...
    const viWords = words.filter(w => VI_DIACRITIC.test(w)).length;
    // Ngưỡng 0.30: câu tiếng Việt mật độ dấu thường >0.5; câu tiếng Anh chỉ lẫn vài
    // tên riêng có dấu ("Công an Thanh Miếu") vẫn ở dưới ngưỡng → phân loại đúng 'en'.
    return (viWords / words.length) >= 0.30 ? 'vi' : 'en';
}
...
function gradeGrounding(expectation, evalTrace, result) {
    ...
    for (const fact of expectation.required_facts || []) {
        ...
        const groundedInDocs = factMatches(fact, matchedDocs);
