...
// --------------------------------------------------------------------
// Phân loại lỗi hạ tầng/provider (runner-reliability fix): tách INFRA/PROVIDER
// failure (SERVER_CONFIG_ERROR, timeout Pinecone, lỗi HTTP provider…) khỏi
// CODE/CONTENT regression thật (HARD_FAIL) — để report/gate không đọc nhầm
// một lỗi hạ tầng thoáng qua thành regression code (xem docs/brain/03-decisions.md).
// --------------------------------------------------------------------
function classifyInfraErrorCode(result) {
    // Lỗi tường minh handler trả về (SERVER_CONFIG_ERROR, RATE_LIMIT, API_ERROR,
    // STREAM_ERROR, UNKNOWN_ERROR, BAD_REQUEST…) — nguồn đáng tin nhất, ưu tiên trước.
    if (result.error) return String(result.error);
    // Pinecone lỗi/timeout bị api/chat.js NUỐT (catch) để bot vẫn trả lời — không có
    // result.error nhưng eval trace (chỉ có ở regression/eval run) lộ cờ pineconeErrored.
    const evalTrace = result.eval || null;
    if (evalTrace && evalTrace.pineconeErrorDetail) {
        return /timeout/i.test(String(evalTrace.pineconeErrorDetail)) ? 'PINECONE_QUERY_TIMEOUT' : 'PINECONE_ERROR';
    }
    if (evalTrace && evalTrace.pineconeErrored) return 'PINECONE_ERROR';
    return null;
}
...
function classifyVerdict(expectation, deterministic, grounding) {
    ...
    let verdict;
    if (deterministic.providerError && failures.length === 0) {
        // Lỗi provider/infra thật (SERVER_CONFIG_ERROR…) khiến không có nội dung để chấm —
        // KHÔNG BAO GIỜ được coi là PASS chỉ vì "không tìm thấy fact nào thiếu" trên câu
        // trả lời rỗng. Đây chính là false-green đã phát hiện sau PR #59.
        verdict = 'INFRA_FAIL';
...
function gradeCase(expectation, result, options = {}) {
    ...
    // infraErrorCode có thể đi kèm HARD_FAIL (Pinecone timeout làm sources=[] nhưng model
    // vẫn sinh câu trả lời ungrounded) — KHÔNG che HARD_FAIL, chỉ đính thêm bằng chứng hạ
    // tầng để report/gate phân biệt được CODE regression khỏi INFRA/PROVIDER failure.
    const infraErrorCode = classifyInfraErrorCode(result);
    return {
