...
const userListRaw = `Công an tỉnh Phú Thọ
...
const userList = userListRaw.split('\n').map(s => s.trim().replace(/xã/g, 'Xã').replace(/phường/g, 'Phường')).filter(Boolean);
const missing = masterList.filter(item => !userList.includes(item));
const unknown = userList.filter(item => !masterList.includes(item));

let o = 'Danh sách chưa điền:\n' + missing.join('\n') + '\n\n';
o += 'Danh sách các đơn vị lạ:\n' + unknown.join('\n') + '\n';

fs.writeFileSync('report2.txt', o, 'utf8');
