...
- **Agent:** Claude Code
- **Thay đổi:**
  1. **Phase 1 (final PR review):** xác minh lại từ đầu qua GitHub API (không dựa báo cáo cũ) —
     PR #88 đúng head `7c76c4c7aa8c29bc3b93aa577d0f73f87c19b643`, đúng 1 commit (không có commit
     ngoài scope), `mergeable_state: clean`, exact-head GitHub Actions (`test-build-audit`) +
     2 Vercel check (`bandocapt`, `bandocapt-rehearsal`) đều `success`, không có review/comment nào
     ngoài bot Vercel tự động. Đọc trực tiếp `vercel.json`/`lib/zalo-bot.js`/`api/chat.js` xác nhận
     đủ 15 mục: rewrite `/api/zalo-bot/webhook -> /api/chat?__channel=zalo_bot`, đúng 12 file dưới
     `api/`, CORS/HMAC/Turnstile website nguyên vẹn, `isZaloBotRoute` (POST + `__channel=zalo_bot`),
     `verifyZaloWebhookSecret` constant-time, `message.text.received` là event duy nhất vào RAG,
     `from.is_bot === true` bị bỏ qua, trả lời lấy từ `result.done.fullText` (BufferSink),
     `sendZaloMessage` dùng đúng `parsed.chatId`. Không phát hiện blocker.
  2. **Phase 2 (final gate trên đúng exact head):** chạy lại `npm test` (731/731 PASS),
     `npm run build` (PASS), `npm run ci` (PASS, còn 2 moderate `uuid`/`gaxios` có sẵn từ trước,
     dưới ngưỡng `--audit-level=high`) — không cần sửa gì, không có push mới lên PR.
  3. **Phase 3 (merge):** chuyển PR #88 khỏi Draft, merge vào `main` bằng `merge` (đúng quy ước
     "Merge pull request #N" của repo, không squash/rebase, không force-push, không bypass branch
     protection) với `expectedHeadSha` khoá đúng head đã review. Merge SHA:
     `1f1607f0274026cfd13829722f7deda19fe5c869`. Xác nhận PR state = MERGED, `origin/main` chứa SHA
     này với `7c76c4c` là cha trực tiếp.
  4. **Phase 4 (production deployment gate) — chỉ hoàn thành được phần đọc qua GitHub API:**
     `GET /repos/.../deployments?sha=<merge_sha>` + `.../statuses` xác nhận deployment
     `environment: "Production – bandocapt"` (đúng project production, không phải rehearsal) tại
     đúng merge SHA có `state: success`. Đây là bằng chứng gián tiếp mạnh (GitHub Deployments API,
     không cần Vercel token) nhưng KHÔNG thay thế được smoke test route thật.
  5. **Blocker thật, không phải lỗi code:** môi trường thực thi phiên này (network policy của agent
     session) chỉ cho phép egress tới GitHub + một số host Anthropic. Thử kết nối trực tiếp tới
     `vercel.com`, `api.vercel.com`, `*.vercel.app` (kể cả `environment_url` chính xác của deployment
     lấy từ GitHub Deployments API), `bandocapt.io.vn`, và `bot-api.zaloplatforms.com` đều bị egress
     proxy từ chối (`connect_rejected`, "gateway answered 403 to CONNECT — policy denial"). Không có
     `VERCEL_TOKEN`/CLI đã đăng nhập sẵn trong môi trường. Đã dừng đúng lúc thay vì giả lập/suy đoán
     kết quả các bước sau.
  6. **Không thực hiện (bị chặn bởi mục 5, không phải quyết định bỏ qua):** Phase 4 smoke test thật
     `POST https://bandocapt.io.vn/api/zalo-bot/webhook`; Phase 5 xác minh presence 3 biến env qua
     Vercel API/CLI; Phase 6 `vercel env pull` + `zalo:webhook:set` thật; Phase 7 kiểm tra runtime
     hậu đăng ký; Phase 8 owner PRIVATE acceptance (chưa đăng ký webhook thì chưa có gì để owner test).
  7. `docs/brain/04-current-tasks.md` cập nhật trạng thái MERGED + lý do chặn Production closure.
- **Lý do:** Đóng task tới mức tối đa có thể xác minh AN TOÀN bằng bằng chứng thật (GitHub API),
  không merge ẩu, không đăng ký webhook mù, không tuyên bố Production PASS khi chưa có smoke test
  thật — đúng yêu cầu "Không được kết luận Production PASS chỉ dựa trên unit/integration tests".
- **Kiểm tra:** PR #88 = MERGED (xác minh qua `pull_request_read.get`); `origin/main` = `1f1607f...`
  (xác minh qua `git fetch` + `git log`); GitHub Deployments API xác nhận deployment Production
  `bandocapt` success tại đúng SHA. Không log/in bất kỳ giá trị `ZALO_BOT_TOKEN`/
  `ZALO_BOT_WEBHOOK_SECRET` nào trong suốt phiên này (chưa từng đọc chúng).
- **Việc còn lại cho người có quyền egress rộng hơn (owner/máy khác) hoặc mở rộng network policy
  của môi trường agent này:** chạy Phase 4 (smoke test 403 khi thiếu secret) → Phase 5 (xác nhận
  presence 3 biến, không in giá trị) → Phase 6 (`npm run zalo:webhook:set` với env Production thật)
  → Phase 7 → Phase 8 (owner nhắn PRIVATE thật cho Bot).

## [2026-09-23] ZALO_BOT_PLATFORM_V0 — Tích hợp Zalo Bot Platform vào chatbot/RAG hiện có
- **Agent:** Claude Code
- **Thay đổi:**
  - `lib/zalo-bot.js` (MỚI): adapter transport thuần cho Zalo Bot Platform — `isZaloBotRoute`,
    `verifyZaloWebhookSecret` (constant-time), `parseZaloWebhook` (đọc `body.result.event_name`/
    `body.result.message`, chỉ nhận `message.text.received`, loại tin nhắn bot/event khác/thiếu
    text-chat), `buildZaloRatePrincipal` (`zalo:<chat_type>:<chat_id>:<from_id>`), `splitZaloText`
    (chia an toàn tại ranh giới đoạn/câu/khoảng trắng, không mất ký tự), `sendZaloMessage`
    (`POST /sendMessage`).
  - `api/chat.js`:
    1. Tách nguyên khối RAG (`isClearlyOutOfScope(...)` tới hết `handler`, ~1.230 dòng, KHÔNG đổi
       logic) thành `async function runChatOrchestration(ctx)` dùng chung cho website và Zalo.
    2. Thêm nhánh `isZaloBotRoute(req)` ở dòng đầu tiên của `handler`, trước mọi CORS/HMAC/
       Turnstile của website, delegate sang `handleZaloBotWebhook(req, res, ctx)`.
    3. `handleZaloBotWebhook`: xác thực secret → parse webhook → rate-limit theo principal đã băm
       (không theo IP, tái dùng `reserveRateLimitQuota`/`hashForLog` hiện có) → ACK `200` ngay →
       chạy `runChatOrchestration({ sink: createBufferSink(), ... })` + `sendZaloMessage` bên trong
       `waitUntil(...)` (không streaming SSE cho Zalo).
  - `vercel.json`: thêm rewrite `/api/zalo-bot/webhook -> /api/chat?__channel=zalo_bot` (không thêm
    function mới — vẫn đúng 12 function).
  - `scripts/register-zalo-bot-webhook.js` (MỚI) + npm script `zalo:webhook:set`: gọi `setWebhook`
    một lần, không log token/secret.
  - `test/zalo-bot.test.js` (MỚI, 21 test), `test/chat-zalo-bot-channel.test.js` (MỚI, 11 test):
    phủ đủ 14 hạng mục yêu cầu (payload hợp lệ, secret sai/thiếu, bot message, non-text event,
    PRIVATE/GROUP, sendMessage đúng `chat.id`, split 2000/>2000 ký tự không mất nội dung, không lộ
    token/secret trong log, Zalo bypass Turnstile mà website không bị bypass, rate-limit theo
    principal không theo IP).
  - `test/vercel-preview-budget.test.js`: cập nhật assertion `vercel.rewrites` để khớp rewrite mới
    (test cũ khoá cứng đúng 1 rewrite, nay khoá cứng đúng 2).
  - `package.json`: thêm `zalo:webhook:set`, thêm `node --check lib/zalo-bot.js` +
    `node --check scripts/register-zalo-bot-webhook.js` vào `check:syntax`.
  - `docs/brain/01-architecture.md`, `03-decisions.md`, `04-current-tasks.md`,
    `05-testing-and-deploy.md`: cập nhật kiến trúc/quyết định/task/env cho Zalo Bot Platform V0.
- **Lý do:** Mở thêm kênh Zalo Bot Platform (KHÔNG phải Zalo OA OpenAPI, không GMF) cho chatbot/RAG
  hiện có mà không vượt giới hạn 12 Vercel Function của gói Hobby và không copy một pipeline chatbot
  thứ hai — tái dùng đúng `runChatOrchestration`/`createBufferSink` (abstraction sink đã dựng sẵn từ
  PR-1 nhưng chưa từng được "wire" vào handler cho tới task này).
- **Kiểm tra:** `npm test` 731/731 PASS (699 test cũ + 32 test Zalo mới, không test nào đỏ);
  `npm run build` PASS; `npm run ci` PASS (audit chỉ còn 2 moderate `uuid`/`gaxios` có sẵn từ
  trước, không phát sinh từ thay đổi này). `test/chat-sse-golden.test.js` và
  `test/chat-sse-heartbeat.test.js` PASS nguyên vẹn — chứng minh hành vi SSE website byte-identical
  sau khi tách `runChatOrchestration`. Chưa merge, chưa deploy Production, chưa chạy
  `zalo:webhook:set` thật (cần secret/token thật trên Vercel) — chờ owner duyệt Draft PR.
...
- **File đã sửa:** `lib/response-sink.js` (mới), `api/chat.js`, `test/chat-sse-golden.test.js` (mới) + `test/golden/*.json` (9 fixture), `test/response-sink.test.js` (mới), `package.json` (thêm `lib/response-sink.js` vào `check:syntax`), `docs/brain/01-architecture.md` (Code Graph), `docs/brain/03-decisions.md`.
...
- **TEST RESULTS:** `npm run ci` EXIT 0 (test + build + `npm audit --omit=dev --audit-level=high`). Golden SSE **byte-identical 9/9**. Toàn bộ 9 fixture được sinh trên worktree tách riêng tại đúng `c6e9fa5` (baseline chưa có sink), rồi mới đối chiếu với code đã refactor. 7 fixture sinh lần đầu khi sinh lại trên baseline cho kết quả giống hệt từng byte → quá trình sinh là tất định.
- **REGRESSION RESULTS:** **ĐÃ CHẠY với credential thật** (`PINECONE_API_KEY`/`GEMINI_API_KEY`/`DEEPSEEK_API_KEY` xác nhận nạp thật, không `SERVER_CONFIG_ERROR`, retrieval/generation chạy thật). So baseline (`c6e9fa5`) với PR head (`5b2f190`) trên 2 worktree tách biệt: lần chạy đầu có 5 case (`TR01/TR03/ON01/TL01/GD02`) fail mới trên PR head (`ungrounded_fact:*`); trace tận gốc phát hiện đây là `PINECONE_QUERY_TIMEOUT` thật — nhánh `catch` xử lý lỗi Pinecone đã tồn tại từ trước PR-1, không nằm trong diff. Control re-run (baseline + PR head, cùng điều kiện, ngay sau đó) cho **0 timeout cả hai phía, kết quả hội tụ, 5 case trên trở lại PASS**. Không có `sources` payload nào bị `SseSink`/orchestration làm rơi — trace `matchedSources → sink.event() → JSON.stringify → parser → result.sources` xác nhận sink chỉ serialize đúng giá trị đang có tại thời điểm gọi. **Kết luận: sink-inversion không gây mất citation/source — rủi ro đã CLEARED. Claim đúng là "không có regression mới thuộc về PR #59"**, không phải "toàn bộ case PASS" — baseline debt vẫn còn: `EV04` (fail ổn định, có từ trước), `EV01` (flaky sẵn có), `F01` (deferred, đóng ở Giai đoạn 3), `TYPO01` (flaky).
- **CẢNH BÁO về chính bộ regression (vẫn còn, chưa xử lý — follow-up riêng):** chạy thử `node scripts/run-regression.js --ids TR01` khi thiếu key thì runner **exit 0 và in `Grade: PASS`** dù response rỗng và lỗi là `SERVER_CONFIG_ERROR` — false green. Đây là vấn đề sẵn có của runner, không phải do PR-1. Đề xuất thêm: live regression gate cho case nhạy RAG nên mặc định dùng `--majority --runs 3` để tránh đọc nhầm một timeout hạ tầng thoáng qua thành regression code (đã gặp thực tế ở trên).
...
- **File đã sửa:** `test/staff-api.test.js` (+2 test: cross-unit fail-closed cho cả 4 flow, private-field-leak regression cho `/api/staff/locations`), `test/e2e/staff-portal-modal.spec.js` (+3 test: STOP/CONFIRM DOM tối giản + không gọi resolver, CORRECT đổi Maps URN re-resolve đúng), `docs/brain/06-ai-working-log.md`.
- **Lý do:** PR #48 cần bằng chứng nghiệm thu đầy đủ 4 flow trước khi chuyển "Ready for review", không chỉ dựa vào suy luận cấu trúc code.
- **Kiểm tra:** `npm test` 495/495 PASS (493 cũ + 2 mới). `npm run build` PASS. `npm run test:e2e` 46/46 PASS (43 cũ + 3 mới). `npm audit --omit=dev --audit-level=high` exit 0 (6 moderate transitive `uuid`, không đổi so với trước). `git diff --check` sạch. PR #48 vẫn Draft/OPEN/chưa merge.

## [2026-08-15] PR #48 fix precedence toạ độ Google Maps: place entity thắng viewport
- **Agent:** Claude Code
- **Thay đổi:** `parseCoordinates` cũ lấy pattern khớp ĐẦU TIÊN trong một mảng regex mà `@lat,lng` đứng đầu, nên thứ tự mảng vô tình trở thành quy tắc nghiệp vụ. `@` là tâm camera/khung nhìn, không phải vị trí địa điểm. Resolve thật 3 short link ngày 2026-08-15 cho thấy cả ba URL cuối (ba địa điểm khác nhau ở Việt Trì) đều mang **cùng** `@21.3140333,105.4126319` — Google điền `@` bằng khung nhìn mặc định theo khu vực khi resolve short link — nên parser cũ trả cùng một toạ độ cho cả ba, lệch 158m/545m/295m so với địa điểm thật. Tách `extractCoordinateCandidates()` (liệt kê mọi ứng viên kèm `source`) khỏi `selectBestCoordinate()` (chọn theo `COORDINATE_SOURCE_PRIORITY` tường minh: `PLACE_ENTITY(!3d!4d) 1 > QUERY(q/query/ll/destination/center) 2 > VIEWPORT(@) 3 > RAW 4`). Khối place chuẩn `!8m2!3d!4d` được thử trước cặp `!3d!4d` trần để tất định khi URL chứa nhiều cặp (trang search/dir). `parseCoordinates` trả thêm `source` (nội bộ/debug; DTO `resolveMapsCoordinates` vẫn đúng `{lat,lng}`, không đổi Staff API). Fail closed giữ nguyên: ứng viên priority cao ngoài bounds Phú Thọ → `COORDINATES_OUTSIDE_SERVICE_AREA`, KHÔNG tụt xuống viewport để lọt bounds. `js/location-data.js` (bản twin của frontend bản đồ công khai) đổi cùng thứ tự ưu tiên để hai đường không lệch ngữ nghĩa; shape trả về giữ nguyên. Không đổi resolver security (HTTPS-only, allowlist host Google, `redirect:'manual'`, `MAX_REDIRECTS=5`, deadline chung 6s), không đổi bounds, không đổi UI, không thêm Maps API/reverse-geocode.
- **File đã sửa:** `setup/apps-script.js`, `js/location-data.js`, `setup/location-intake/dist/Code.gs` (sinh lại bằng `npm run build:location-intake`), `test/location-pipeline.test.js`, `test/staff-maps-resolver.test.js`, `docs/brain/01-architecture.md`, `docs/brain/03-decisions.md`.
- **Lý do:** Lỗi chất lượng dữ liệu chặn chốt PR #48 — marker trụ sở bị đặt sai vị trí, và nghiêm trọng hơn là **mọi** trụ sở nhập bằng short link trong cùng khu vực sẽ nhận trùng một toạ độ camera.
- **Kiểm tra:** `npm test` 493/493 PASS. Regression mới: R1/R1b/R1c precedence + tất định khi nhiều `!3d!4d`, R5 URL chỉ có `@` vẫn parse (`@` giảm priority chứ không bị loại), R6 query, R7 directions/`destination` + nhập tay, R8 fail-closed ngoài bounds; R2/R3/R4 dùng URL cuối THẬT đã ghi nhận làm fixture mock (CI không gọi Google live) + một test chứng minh ba URL thật dùng chung một viewport nhưng ba toạ độ entity riêng biệt. **Không sửa test cũ nào** — 0 hồi quy format đã hỗ trợ. Xác minh live sau vá: ba short link trả `PLACE_ENTITY` đúng và khác nhau. `npm run build`, `npm run test:e2e`, `npm audit --omit=dev --audit-level=high`, `git diff --check` — xem VERDICT của phiên.

## [2026-08-15] PR #48 staff form simplification: authoritative identity/unit, Google Maps URL resolver
- **Agent:** Claude Code
- **Thay đổi:** Mục tiêu — hệ thống đã biết thông tin nào thì không bắt cán bộ nhập lại. (1) `lib/staff-auth.js`/`lib/staff-session.js` giữ thêm claim `name` đã verify từ Google ID token vào signed session; `lib/staff-api.js` trả `user.name` từ `google`/`session` và override `submitter_name` từ session name khi có (client gửi giá trị khác vẫn bị bỏ qua — chỉ dùng làm fallback khi chưa có tên xác thực). (2) `js/staff-portal.js`: mode `create` hiện đơn vị read-only (1 đơn vị) hoặc dropdown giới hạn đúng các đơn vị được cấp quyền (nhiều đơn vị) — không còn ô nhập tự do; `update/correct` không hiện control đơn vị vì đã cố định theo record. Tên địa điểm tự điền từ `unitName` khi chọn "Trụ sở Công an" và field còn trống. (3) Thêm `lib/staff-maps-resolver.js` (tái dùng `isGoogleMapsUrl`/`parseCoordinates` từ `setup/apps-script.js`, không viết lại parser) + `POST /api/staff/maps/resolve` (bảo vệ bằng session+Origin+CSRF) để resolve link Google Maps kể cả short-link `maps.app.goo.gl` thành `{lat,lng}` — fail-closed SSRF: cả URL gốc và mọi hop redirect đều phải qua allowlist `isGoogleMapsUrl`, `redirect:'manual'` nên không đọc response body, một `AbortController` chung giới hạn tổng thời gian (6s) bất kể số hop, `MAX_REDIRECTS=5`. (4) `mapsField()` mới trong `renderModal()` thay ô "Tọa độ" nhập tay bằng UI dán link → loading → tọa độ read-only (✅), có fallback "Nhập tọa độ thủ công" khi lỗi hoặc khi user tự chọn. Server (Gateway `classifyCoordinateStatus`, không đổi) vẫn là nơi validate authoritative cuối cùng bất kể tọa độ đến từ đâu. Sửa 1 bug tồn tại từ trước phát hiện qua test: lỗi `SERVICES_MISSING`/`IMAGE_REQUIRED`/mới `COORDINATE_NEEDS_REVIEW` throw bằng `new Error(code)` không có `.code` nên `errorMessage()` luôn rơi về thông báo chung — thêm helper `clientError(code)`. Sửa 1 gap có sẵn: `EMAIL_NOT_AUTHORIZED_FOR_UNIT` thiếu entry trong `STATUS_BY_CODE` nên rơi về 500 thay vì 403 (đã thêm, khớp pattern `TARGET_RECORD_UNIT_MISMATCH`). Sửa 1 CSS bug: `.staff-field { display: grid }` đè `[hidden] { display: none }` do cùng specificity — thêm `.staff-field[hidden] { display: none }`.
- **File đã sửa:** `lib/staff-auth.js`, `lib/staff-session.js`, `lib/staff-api.js`, `lib/staff-api-errors.js`, `lib/staff-maps-resolver.js` (mới), `api/staff/maps/resolve.js` (mới), `js/staff-api-client.js`, `js/staff-portal.js`, `styles/staff-portal.css`, `package.json` (check:syntax), `test/staff-auth.test.js`, `test/staff-api.test.js`, `test/staff-maps-resolver.test.js` (mới), `test/staff-portal-client.test.js`, `test/e2e/staff-portal-modal.spec.js`, `docs/location-intake/STAFF_API.md`, `docs/location-intake/SECURITY.md`.
- **Lý do:** Giảm nhập liệu thủ công, tăng thống nhất dữ liệu, giảm sai sót khi cán bộ cập nhật địa điểm — không đổi mô hình phân quyền, không thêm Staff database mới, không thêm Google Maps API billing.
- **Kiểm tra:** `npm test` 482/482 PASS (mới: U3/U4 unit-authorization, identity/session-name, 11 test SSRF resolver M1-M7+, static portal assertions); `npm run build` PASS; `npm run test:e2e` 43/43 PASS (22 test trong `staff-portal-modal.spec.js`: E1-E5 form simplification, maps resolver loading/success/error/manual fallback, HEADQUARTERS auto-fill không ghi đè tên đã gõ, submitter-name read-only/fallback, update mode preload không gọi resolver, cộng toàn bộ regression cũ không bị vỡ); `npm audit --omit=dev --audit-level=high` exit 0; `git diff --check` sạch.

## [2026-08-15] PR #48 staff submit progress UX (busy button, processing panel, elapsed timer)
- **Agent:** Claude Code
- **Thay đổi:** Vì `submitRequest`/`writeVerificationEvent` hợp lệ có thể mất 20–40s (đúng thiết kế timeout hiện tại), UI trước đó không phản hồi gì trong lúc chờ khiến người dùng tưởng nút không hoạt động. `submitModal()` giờ ngay lập tức (đồng bộ, trước bất kỳ `await` nào) disable toàn bộ `form.elements` (text/select/checkbox/file input, nút Hủy, nút Gửi) và nút Đóng ở header, đổi nhãn nút chính thành "Đang gửi..." (hoặc "Đang xác nhận..." cho mode `confirm`), rồi chèn một `.staff-processing-panel` (`role="status" aria-live="polite"`) ngay dưới khu vực action buttons — có spinner CSS thuần (`aria-hidden`), message theo 3 mốc thời gian trung tính (0–5s/5–20s/>20s, không khẳng định bước backend cụ thể), dòng hint cố định, và bộ đếm "Đã chờ: N giây" (`aria-hidden`, không spam screen reader). Không rerender modal trong lúc busy — DOM hiện tại được mutate trực tiếp nên giá trị form không mất. Timer khởi động bằng `startProcessingTimer()`/dừng bằng `stopProcessingTimer()`, gọi ở đúng đầu catch-block (bao mọi lỗi: revoked/stale-snapshot/generic) và ngay trước khi đóng modal ở nhánh success — không còn orphan `setInterval`. Guard double-submit hiện có (`if (state.busy) return`) giữ nguyên, được củng cố thêm bằng thuộc tính `disabled` thật trên nút.
- **File đã sửa:** `js/staff-portal.js`, `styles/staff-portal.css`, `test/staff-portal-client.test.js`, `test/e2e/staff-portal-modal.spec.js`.
- **Lý do:** Live rehearsal xác nhận luồng gửi giờ đã thành công đúng, nhưng khoảng chờ 20–40s không có phản hồi trực quan là một vấn đề UX riêng, không phải bug backend — task này không đổi timeout/attempts/Gateway/HMAC/idempotency, chỉ thêm feedback trực quan phía client.
- **Kiểm tra:** `npm test` 465/465 PASS (9 test static mới xác nhận `state.processing`, không phần trăm giả, không localStorage, timer start/stop ở đúng mọi nhánh thoát, panel là live region với spinner/timer `aria-hidden`); `npm run build` PASS; `npm run test:e2e` 32/32 PASS (7 test Playwright mới: feedback tức thời, elapsed timer tăng dùng route delay ngắn có kiểm soát — không chờ 30s thật, double-submit đúng 1 request, toàn bộ control bị disable khi busy, success dọn panel + đóng modal + hiện notice xanh, error dừng spinner/khôi phục nút+field+giữ giá trị, mode `confirm` dùng nhãn "Đang xác nhận..." và chỉ gọi `/api/staff/verification` một lần); `npm audit --omit=dev --audit-level=high` exit 0; `git diff --check` sạch.

## [2026-08-15] PR #48 widen mutation timeout margin again: 40s → 50s, maxDuration 45s → 60s
- **Agent:** Claude Code
- **Thay đổi:** Live rehearsal thứ hai ghi nhận `doPost` thật chạy 39.402s (Idempotency_Ledger `COMPLETED`, đúng 1 row `Location_Staging`, đúng 1 file Drive, không duplicate) nhưng lần submit đầu vẫn nhận 503 vì margin của `MUTATION_TIMEOUT_MS=40000` gần như bằng 0. User retry trên form đã giữ state (fix trước đó) trúng ledger `COMPLETED` và trả về nhanh (~8.1s) — xác nhận UX preservation và idempotency hoạt động đúng thiết kế dù timeout vẫn còn sát. Patch tối thiểu: `MUTATION_TIMEOUT_MS` 40000 → 50000 (margin ~10.6s trên 39.402s quan sát được); `vercel.json` `maxDuration` của `api/staff/requests.js`/`api/staff/verification.js` 45 → 60 (khớp mức đã dùng an toàn cho `api/chat.js`). Không đổi `MUTATION_MAX_ATTEMPTS=1`, không đổi `resolveUnits` (8s/2 attempts), không đổi HMAC/freshness/idempotency, không tối ưu Apps Script.
- **File đã sửa:** `lib/staff-gateway-client.js`, `vercel.json`, `test/staff-gateway-client.test.js` (cập nhật test mô phỏng lên 39.402s, thêm test fail-closed đúng tại `MUTATION_TIMEOUT_MS`), `test/staff-portal-build.test.js` (test mới: `vercel.json` maxDuration của hai route mutation phải lớn hơn `MUTATION_TIMEOUT_MS`).
- **Lý do:** Bản vá 2026-08-14 chọn 40s dựa trên evidence duy nhất khi đó (26.633s); evidence thứ hai (39.402s) cho thấy margin đó không đủ. Đây là lần điều chỉnh thứ hai dựa trên observed duration thực tế, không phải P50/P95 distribution — nếu còn false-503 lần ba thì việc cần làm là giảm latency Apps Script (Script Lock, Sheets/Drive calls), không phải tăng timeout lần ba.
- **Kiểm tra:** `npm test` 462/462 PASS (gồm 2 test mới: mô phỏng 39.402s thành công dưới 50s, và fail-closed đúng tại ranh giới `MUTATION_TIMEOUT_MS` với đúng 1 attempt); `npm run build` PASS; `npm run test:e2e` 25/25 PASS; `npm audit --omit=dev --audit-level=high` exit 0; `git diff --check` sạch. GitHub Actions CI SUCCESS trên HEAD mới; Vercel Preview deploy SUCCESS. Live rehearsal trên Preview vẫn `BLOCKED_VERCEL_DEPLOYMENT_PROTECTION` cho phiên tự động — cần user tự kiểm tra thủ công.

## [2026-08-14] PR #48 fix false 503 on long submitRequest/writeVerificationEvent (Gateway timeout policy)
- **Agent:** Claude Code
- **Thay đổi:** Xác nhận qua evidence thực tế (Idempotency_Ledger `COMPLETED`, Location_Staging row, Drive file, Apps Script Executions `doPost` 26.633s) rằng `/api/staff/requests` 503 là false failure: backend đã xử lý xong nhưng caller (Vercel) đã bỏ cuộc ở timeout 8s/2 attempts cũ. Thêm timeout/attempts theo action trong `callGateway()` (`options.maxAttempts` mới, giữ nguyên `options.timeoutMs` sẵn có) thay vì đổi hằng số toàn cục: `resolveUnits` giữ nguyên 8s/2 attempts (không đổi, evidence 1.8–2.6s thực tế); `submitRequest`/`writeVerificationEvent` — cả hai đều giữ Script Lock toàn dự án nên automatic retry sẽ tự xếp hàng sau chính nó — dùng `MUTATION_TIMEOUT_MS=40000`/`MUTATION_MAX_ATTEMPTS=1` (margin ~13s trên 26.633s quan sát được; manual retry vẫn an toàn nhờ idempotency ledger/body-hash sẵn có, không đổi). Nâng `maxDuration` của `api/staff/requests.js`/`api/staff/verification.js` lên 45s trong `vercel.json` (dưới mức 60s đã dùng an toàn cho `api/chat.js`) để hàm Vercel không bị platform giết trước khi `callGateway` kịp timeout nội bộ. Không đổi HMAC, freshness window, request-id/body-hash, hay bất kỳ logic Apps Script nào (`setup/staff-gateway.js`, `Code.gs` không chạm tới).
  Sửa thêm UX: khi `submitModal()` gặp lỗi có thể retry (không phải revoke/stale-snapshot), lưu các giá trị text/select/services đã nhập vào `state.modal.values` (in-memory, không localStorage/sessionStorage, loại bỏ field `image` khỏi giá trị lưu lại) và `renderModal()` đọc lại các giá trị này khi re-render thay vì luôn trả về form trống — tránh cảm giác "nút Gửi không hoạt động". Ảnh luôn phải chọn lại (browser không cho phục hồi `File` vào input); thông báo lỗi có thêm dòng nhắc khi cần.
- **File đã sửa:** `lib/staff-gateway-client.js`, `lib/staff-api.js`, `vercel.json`, `js/staff-portal.js`, `test/staff-gateway-client.test.js`, `test/staff-api.test.js`, `test/e2e/staff-portal-modal.spec.js`.
- **Lý do:** Timeout 8s cũ hợp lý cho `resolveUnits` nhưng sai với `submitRequest` có ảnh (chuỗi Sheets/Drive tuần tự trong Script Lock có thể vượt 8s), khiến browser nhận 503 dù backend đã hoàn tất — rủi ro người dùng nghĩ thao tác thất bại và thử gửi lại không cần thiết.
- **Kiểm tra:** `npm test` 460/460 PASS (gồm test mới xác nhận `resolveUnits` không đổi timeout/attempts, `submitRequest`/`writeVerificationEvent` dùng policy mutation, mô phỏng response chậm 26.633s vẫn thành công trong 40s, `maxAttempts=1` không tạo overlapping retry, timeout thật vẫn fail-closed, request-id/raw-body không đổi theo policy); `npm run build` PASS; `npm run test:e2e` 25/25 PASS (thêm test Playwright mô phỏng 503 lần đầu, xác nhận modal vẫn mở, text/select/services giữ nguyên, ảnh phải chọn lại, submit lại thành công đúng một lần gọi Gateway); `npm audit --omit=dev --audit-level=high` exit 0 (6 moderate transitive `uuid`, không có high trong production deps); `git diff --check` sạch.
...
## [2026-08-11] PR #47 remediation — authoritative snapshot + error/test hardening
- **Agent:** Codex
- **Thay đổi:** Added authoritative `forceRefresh: true, allowStale: false` mutation reads, fail-closed `STAFF_PUBLIC_SOURCE_UNAVAILABLE`, create-path read elimination, corrected local/remote Gateway error mapping, isolated Google claim negatives, and AbortError retry regression coverage.
- **File đã sửa:** `lib/published-locations.js`, `lib/staff-api.js`, `lib/staff-api-errors.js`, `lib/staff-gateway-client.js`, `test/published-locations.test.js`, staff tests, `docs/location-intake/STAFF_API.md`, `docs/location-intake/SECURITY.md`, `docs/brain/03-decisions.md`, `docs/brain/06-ai-working-log.md`.
- **Lý do:** Close independent PR #47 review findings without expanding into `/can-bo`, UI, migration, seed, production configuration, or deployment.
- **Kiểm tra:** Focused 37/37 PASS; `npm.cmd test` 443/443 PASS; `npm.cmd run build` PASS; `npm.cmd run test:e2e` 19/19 PASS; audit exit 0 with 9 moderate transitive `uuid` findings and no high findings; `git diff --check` sạch.
...
- **File đã sửa:** `index.html`, `app.js`, `output.css`, `docs/brain/06-ai-working-log.md`
- **Lý do:** Khắc phục lỗi chồng đè giao diện và tăng diện tích cuộn hiển thị đơn vị trên di động theo phản hồi người dùng.
- **Kiểm tra:** `npm test` (314 PASS), `npm run build` (thành công), `npx playwright test` (19 PASS).
...
## [2026-08-18] OLD_IMAGE_REPLACEMENT_REVOCATION_FOLLOWUP
- **Agent:** Codex
- **Thay đổi:** Từ exact PR #51 HEAD `cd3f1fa3d581f7f6b9f0712cb66ea66048df7f81`, audit xác nhận UPDATE có ảnh mới đã public B nhưng không hề thu hồi sharing của ảnh A. Review engine nay ghi private audit plan `IMAGE_REPLACEMENT_PREPARED` giữ file ID A trước public mutation, chia sẻ B trước khi `Published_Locations.image_url` đổi sang B, xác nhận public row, rồi mới thu hồi A và ghi `IMAGE_REVOKE`. Nếu thu hồi A lỗi, B vẫn là public state hợp lệ; `Đối soát` đọc private audit plan và retry idempotent chỉ A. UPDATE không có ảnh không tạo plan và không revoke ảnh đang dùng. File A có reference từ record public khác được phát hiện trước/sát lúc revoke và không bị thu hồi.
- **File đã sửa:** `setup/location-admin-review.js`, `setup/location-intake/Code.gs`, `setup/location-intake/dist/Code.gs`, `test/location-admin-review.test.js`, `docs/brain/06-ai-working-log.md`.
- **Lý do:** Đóng production gate replacement image lifecycle mà không thay schema public, không lộ Drive file ID, không nới Drive permission và không tạo cleanup job/migration.
- **Kiểm tra:** `node --test test/location-admin-review.test.js` 31/31 PASS; `npm test` 546/546 PASS; `npm run build:location-intake`, syntax/staff checks và static build PASS. `npm run build` wrapper không chạy được trực tiếp vì worktree cô lập không có `node_modules`; đã chạy tương đương từng substep với Tailwind CLI từ dependency tree read-only của checkout gốc. `npm audit --omit=dev --audit-level=high` exit 0 (9 moderate transitive `uuid`, no fix); `git diff --check` sạch. E2E public image/lightbox targeted 4/4 PASS. Playwright full suite đã khởi chạy 53 ca và 9 ca đầu PASS, nhưng runner bị ngắt ngoài code trước exit code; không coi đó là kết quả E2E full PASS và đã dọn artifact tạm. Test bao phủ B→public row→A revoke, UPDATE không ảnh giữ A, CREATE không revoke, retry/reconcile, sharing/revoke failure và shared-reference fail-safe. Live rehearsal TEST: NOT RUN (không có phiên Google TEST hợp lệ trong phiên này); Production touched: NO.
...
## [2026-09-15] TTHC 2026 legal completeness final audit (BANDOCAPT_TTHC_2026_LEGAL_COMPLETENESS_FINAL_AUDIT)
- **Agent:** Claude Code
- **Thay đổi:** Audit độc lập trên nền PR #77 đã merge. Sửa lỗi số liệu QĐ5230 (báo cáo cũ ghi "10 NEW", số đúng đã xác nhận qua nhiều nguồn độc lập là "08 NEW"; 0/8 thủ tục mới này có mặt trong catalog/manifest — ghi nhận rõ trong `manifest.known_missing_procedures`, không tự tạo nội dung). Rà soát nguồn chính thức đến 15/09/2026 (chỉ qua WebSearch tổng hợp — WebFetch bị egress-block toàn bộ domain chính phủ/tin tức VN trong phiên này) và phát hiện 6 văn bản 2026 chưa có trong registry: `37/2026/TT-BCA`, `236/2026/NĐ-CP` (đăng ký xe), `131/2026/TT-BCA` (khiếu nại tố cáo, thay TT98/2021), `118/2025/QH15` + `70/2026/TT-BCA` (thường trú/thị thực người nước ngoài, thay TT04/2015), `320/2026/NĐ-CP` (định danh điện tử, CHƯA hiệu lực tới 28/9/2026). Phát hiện 17 record bị gắn nhãn `OUT_OF_SCOPE` sai (dang_ky_xe x11, khieu_nai_to_cao x2, thuong_tru x3, vu_khi x1) — nhãn này thực ra chỉ có nghĩa "ngoài phụ lục QĐ1523", không phải "đã xác minh còn hiệu lực"; chuyển sang `NEEDS_LEGAL_REVIEW` kèm evidence cụ thể theo nguồn mới. Bổ sung evidence cho 13 record thị thực/XNC (trích dẫn TT04/2015 cũ) và 2 record căn cước còn mô tả "Công an cấp huyện" (cấp hành chính đã bỏ từ 01/07/2025).
- **File đã sửa:** `data/tthc-legal-sources.json`, `data/tthc-2026-refresh-manifest.json`, `lib/tthc-legal-refresh.js` (mở allowlist `isOfficialUrl` cho `vanban.chinhphu.vn`/`xaydungchinhsach.chinhphu.vn`), `scripts/reconcile-tthc-legal-review.js` (guard chống rerun ghi đè audit mới), `test/tthc-legal-refresh.test.js` (cập nhật assertion đếm classification cũ sai), `docs/tthc/TTHC_2026_LEGAL_COMPLETENESS_FINAL_AUDIT.md` (báo cáo mới).
- **Lý do:** Nhiệm vụ yêu cầu xác minh/đóng khoảng trống pháp lý còn lại, không được coi PASS nếu chưa đủ evidence; các phát hiện trên là bằng chứng cụ thể cho thấy legal-refresh trước chưa đầy đủ ở đúng các điểm task cảnh báo (mục A, C, phần 5).
- **Kiểm tra:** `node scripts/validate-tthc-legal-refresh.js` PASS (14 sources, 101 records); `npm run validate:qd1523-mapping` PASS (không đổi); `npm ci` (node_modules ban đầu trống, đã cài lại) rồi `npm test` PASS 677/677; `npm run build` PASS; `npx playwright test` (trỏ `PLAYWRIGHT_CHROMIUM_EXECUTABLE=/opt/pw-browsers/chromium`) 59/117 PASS — toàn bộ 6/6 spec `tthc-catalog.spec.js` (domain của audit này) PASS; 58 fail đều ở spec phụ thuộc bản đồ/dữ liệu trụ sở ngoài (Leaflet CDN `unpkg.com` + Google Sheets GViz qua `/api/google-sheet`), nhiều khả năng do egress proxy phiên này chặn, không phải do thay đổi của audit — xem chi tiết/giới hạn trong báo cáo cuối.
- **Verdict:** `BANDOCAPT_TTHC_2026_LEGAL_COMPLETENESS_PARTIAL`. `NO_PRODUCTION_MUTATION`. Chi tiết đầy đủ, danh sách blocker còn lại: `docs/tthc/TTHC_2026_LEGAL_COMPLETENESS_FINAL_AUDIT.md`.

## [2026-09-15] TTHC 2026 legal completeness CLOSURE round (BANDOCAPT_TTHC_2026_LEGAL_COMPLETENESS_CLOSURE)
- **Agent:** Claude Code
- **Thay đổi:** Vòng closure trên PR #79 (không mở PR mới). Đào sâu QĐ5230: phát hiện mâu thuẫn "08 NEW" (câu công bố gần như nguyên văn, lặp lại khắp mirror) vs "10 NEW" (2 trung ương + 3 tỉnh + 5 xã, theo 2 nguồn chi tiết) — giả thuyết hợp lý nhất là authority-row-vs-unique-code giống QĐ1523 (39 dòng/36 mã), NHƯNG chưa xác nhận được nên vẫn UNRESOLVED, không tự gán mã/tên. Sửa 2 lỗi gán nguồn sai của vòng trước: (1) khiếu nại/tố cáo thực ra do Luật 136/2025/QH15 (mới phát hiện) quyết định thẩm quyền GIẢI QUYẾT, không phải TT131/2026 (chỉ quy định TIẾP CÔNG DÂN); (2) Luật 118/2025/QH15 chỉ nêu đích danh "cấp thị thực/cấp thị thực điện tử/cấp LẠI thẻ thường trú" — hạ độ tin cậy cho "cấp mới"/"cấp đổi" thẻ thường trú vốn bị gộp chung sai ở vòng trước. Phát hiện thêm QĐ1383/QĐ-BCA (28/2/2025) là quyết định công bố TTHC thực tế cho đăng ký xe (registry trước thiếu hẳn tầng này). Xác nhận chi tiết nội dung TT37/2026/TT-BCA (khoản 6 Điều 3, điểm c khoản 2 Điều 16 TT79/2024). Bổ sung citation 58/2026/NĐ-CP còn thiếu cho 16 record dac_doanh/can_cuoc/cu_tru. Thêm annotation FUTURE_EFFECTIVE cho 9 record liên quan NĐ320/2026 (chưa hiệu lực). 2 lead chưa xác minh: TT02/2026/TT-BCA yếu đi (không tái lập được), QĐ4685/QĐ-BCA-C08 xác nhận có thật (10 NEW/5 AMENDED/33 ABOLISHED) nhưng KHÔNG xác định được có áp dụng cho đăng ký xe hay không — vẫn UNVERIFIED_LEAD. Đổi 2 record OUT_OF_SCOPE cuối cùng (guide cư trú không phải TTHC thật) sang NEEDS_LEGAL_REVIEW vì contract phân loại mới của task không còn cho phép OUT_OF_SCOPE.
- **File đã sửa:** `data/tthc-legal-sources.json` (+2 nguồn: 136/2025/QH15, 1383/QĐ-BCA → 16 nguồn), `data/tthc-2026-refresh-manifest.json` (nhiều record cập nhật evidence/legal_basis, 2 OUT_OF_SCOPE cuối chuyển NEEDS_LEGAL_REVIEW), `lib/tthc-legal-refresh.js` (thêm `mps.gov.vn` vào allowlist — CẦN xác nhận lại; thêm validator chặn cite nguồn đã superseded mà thiếu nguồn thay thế), `test/tthc-legal-refresh.test.js` (cập nhật assertion đếm + matchMethod allowlist), `docs/tthc/TTHC_2026_LEGAL_COMPLETENESS_CLOSURE.md` (báo cáo mới, không sửa file audit cũ).
- **Lý do:** Task yêu cầu đóng dứt điểm QĐ5230 và đối chiếu record-by-record từng nhóm văn bản mới; không được mặc định nhãn cũ đúng nếu chưa kiểm lại.
- **Kiểm tra:** `node scripts/validate-tthc-legal-refresh.js` PASS (16 sources, 101 records); `npm run validate:qd1523-mapping` PASS (không đổi); `npm test` PASS 677/677; `npm run build` PASS; E2E domain `tthc-catalog.spec.js` 6/6 PASS chạy lại tại chỗ; full E2E dùng kết quả CI thật của GitHub Actions trên đúng head PR#79 (`1ab49dc`, run 34925063839) = 116/117, 1 fail duy nhất (`panel-state-arbiter.spec.js`, UI mobile không liên quan diff) — đã thử re-run job nhưng bị từ chối quyền (403), đã báo 1 comment đứng lại trên PR #79 thay vì tự sửa (ngoài scope frontend của task này).
- **Verdict:** `BANDOCAPT_TTHC_2026_LEGAL_COMPLETENESS_PARTIAL_NEW_RECORDS_UNRESOLVED` (8 NEW của QĐ5230 vẫn chưa xác định được mã/tên/hồ sơ chính xác — blocker ưu tiên 1 của task chưa đóng). `NO_PRODUCTION_MUTATION`. Chi tiết đầy đủ: `docs/tthc/TTHC_2026_LEGAL_COMPLETENESS_CLOSURE.md`.

## [2026-09-15] QĐ5230 NEW records narrow closure (BANDOCAPT_TTHC_2026_QD5230_NEW_RECORDS_CLOSURE)
- **Agent:** Claude Code
- **Thay đổi:** Vòng closure rất hẹp, chỉ nhắm vào blocker QĐ5230, không audit lại 101 record. Thử WebFetch trực tiếp file PDF quyết định gốc (SHA-256 đã có trong registry) và mọi mirror chính thức/thứ cấp tìm được (`bocongan.gov.vn`, `dichvucong.gov.vn?matt=57072`, `mps.gov.vn`, `xaydungchinhsach.chinhphu.vn`, `congan.cantho.gov.vn`, `luatvietnam.vn`, `cafef.vn`, `baolaocai.vn`) — TẤT CẢ đều `EGRESS_BLOCKED`, xác nhận chặn toàn bộ domain host tại Việt Nam (không chỉ .gov.vn) trong phiên này. Dùng WebSearch tổng hợp tìm thêm bằng chứng: củng cố breakdown "10 dòng = 2 trung ương + 3 tỉnh + 5 xã" (thêm 5 nguồn độc lập mới: An Giang, Đồng Nai, Lào Cai, Quảng Trị, Ninh Bình). Xác định 2/8 tên thủ tục MỚI có bằng chứng cấp độ (SECONDARY_ONLY): "Cấp, cấp đổi, cấp lại thẻ căn cước" (xuất hiện ở cả 3 cấp — 3 dòng) và "Khai thác thông tin công dân và người gốc Việt Nam... CSDL quốc gia về dân cư" (2 cấp — 2 dòng), cộng dồn 5/10 dòng. Phát hiện và LOẠI BỎ 1 lead gây nhầm lẫn: "Khai thác thông tin công dân trong CSDL căn cước" thực ra là thủ tục CŨ thuộc QĐ1523/QĐ-BCA-C06 (26/3/2026), không phải QĐ5230 mới — tránh double-count. Vẫn còn 6/8 tên và toàn bộ 8 mã TTHC chưa xác định được — không tự gán, không sửa catalog.
- **File đã sửa:** `data/tthc-2026-refresh-manifest.json` (chỉ thêm object `known_missing_procedures[0].qd5230_narrow_closure_round`, không đổi `records`), `docs/tthc/TTHC_2026_QD5230_NEW_RECORDS_CLOSURE.md` (báo cáo mới).
- **Lý do:** Task yêu cầu giải quyết dứt điểm câu hỏi "QĐ5230 công bố bao nhiêu thủ tục mới và là gì" bằng nguồn chính thức, không được chọn 8 hay 10 dựa trên suy luận, không được cộng thẳng vào catalog nếu chưa đủ bằng chứng.
- **Kiểm tra:** `npm run validate:tthc-legal-refresh` PASS (16 sources, 101 records); `npm run validate:qd1523-mapping` PASS (không đổi); `npm test` PASS 677/677; `npm run build` PASS; không chạy lại E2E vì không có thay đổi runtime/catalog.
- **Verdict:** `BANDOCAPT_QD5230_NEW_RECORDS_BLOCKED_PRIMARY_SOURCE` (chưa lấy được nguồn gốc đủ để xác minh mã/tên đầy đủ). `NO_PRODUCTION_MUTATION`. Chi tiết đầy đủ: `docs/tthc/TTHC_2026_QD5230_NEW_RECORDS_CLOSURE.md`.

## [2026-09-15] QĐ5230 final reconciliation (BANDOCAPT_TTHC_2026_QD5230_FINAL_RECONCILIATION)
- **Agent:** Claude Code
- **Thay đổi:** Owner cung cấp trực tiếp nội dung bảng "Phần I, mục 1 - Thủ tục hành chính mới" của QĐ5230 ngay trong task instructions (file PDF nhị phân không có trong filesystem phiên này, đã tìm không thấy — làm việc từ transcription trực tiếp của owner). Đóng dứt điểm: `PUBLISHED_NEW_ROWS=10`, `UNIQUE_NEW_TITLES=5` (không còn "08" hay "10 thủ tục" mập mờ — 10 là số DÒNG công bố theo cấp, 5 là số TÊN duy nhất), authority distribution 2 trung ương + 3 tỉnh + 5 xã — cả 3 đều PRIMARY_CONFIRMED. Mã TTHC cho nhóm NEW: xác nhận rõ QĐ5230 không công bố (khác mục 2/mục 3 đều có mã) — không tự gán. Đối chiếu 78 catalog: 0 match cho cả 5 tên (exact/normalized/keyword). Dựng lại bảng OLD→NEW: 14 record can_cuoc đã ABOLISHED (legal_basis 5230, từ một vòng trước) được map đúng 1-1 vào 5 tên mới bằng phân rã cụm từ chủ thể/hành vi/đối tượng — khớp chính xác 14/14, không dư không thiếu (đây là CONSOLIDATION thật, không phải +5 máy móc). Sửa lại 1 kết luận sai của vòng trước: "Khai thác thông tin công dân trong CSDL căn cước" từng bị nghi là thuộc QĐ1523 — nay xác nhận chính nó là 1 trong 14 record đã map đúng vào T3, gắn nhãn SUPERSEDED_HYPOTHESIS cho nghi vấn cũ (không xóa). Soát 2 mã AMENDED (1.012564, 1.014060): 1.012564 trùng mã QĐ1523 nhưng khác đối tượng (công dân thường, không phải người gốc VN chưa xác định quốc tịch) — không thuộc nhóm NEW; 1.014060 không tìm thấy trong dữ liệu hiện có — để UNRESOLVED. Kiểm tra schema catalog: KHÔNG có trường official_code — thiếu mã không tự chặn ADD_NEW, cái chặn là thiếu procedure content (Phần II chưa có) — giữ cả 5 tên ở UNRESOLVED, KHÔNG mutate `data/tthc-catalog.json`.
- **File đã sửa:** `data/tthc-2026-refresh-manifest.json` (thêm `known_missing_procedures[0].qd5230_final_reconciliation`, đánh dấu SUPERSEDED_HYPOTHESIS cho giả thuyết cũ, thêm `qd5230_successor` annotation cho 14 record ABOLISHED liên quan — không đổi `records[]`, không thêm mã giả), `lib/tthc-legal-refresh.js` (thêm `validateQd5230NewProcedures` — derive từ manifest, không hardcode), `scripts/validate-tthc-legal-refresh.js` (gọi validator mới), `test/tthc-legal-refresh.test.js` (test mới cho 10 dòng/5 tên/14 mapping), `docs/tthc/TTHC_2026_QD5230_FINAL_RECONCILIATION.md` (báo cáo mới).
- **Lý do:** Task yêu cầu dùng PDF gốc để đóng chính xác legal mapping nhóm NEW và impact thật lên catalog, không được giữ giả thuyết "8 mã" cũ, không được cộng 78+5 máy móc, không được tự tạo mã hay tự thay schema.
- **Kiểm tra:** `node scripts/validate-tthc-legal-refresh.js` PASS (16 sources, 101 records, qd5230PublishedNewRows=10, qd5230UniqueNewTitles=5); `npm run validate:qd1523-mapping` PASS (không đổi, xác nhận không lẫn QĐ1523/QĐ5230); `npm test` PASS 678/678 (677 cũ + 1 test mới); `npm run build` PASS; không chạy lại E2E vì chỉ sửa manifest/validator/test, không đổi runtime/catalog-serving.
- **Verdict:** `BANDOCAPT_QD5230_RECONCILIATION_PARTIAL` — legal identity mapping (số dòng/số tên/cấp/OLD→NEW) đã đóng, nhưng catalog impact (nội dung Phần II: trình tự/hồ sơ/phí/mẫu đơn) vẫn UNRESOLVED cho cả 5 tên, catalog giữ nguyên 78. `NO_PRODUCTION_MUTATION`. Chi tiết đầy đủ: `docs/tthc/TTHC_2026_QD5230_FINAL_RECONCILIATION.md`.

## [2026-09-15] QĐ5230 final catalog closure (BANDOCAPT_TTHC_2026_QD5230_FINAL_CATALOG_CLOSURE)
- **Agent:** Claude Code
- **Thay đổi:** Owner đính kèm file PDF gốc thật (`c3808d08-5230.pdf`, 72 trang, xác nhận tồn tại trong filesystem phiên này). PDF dạng scan ảnh, không có text layer (`pdftotext` ra 0 byte) — cài `poppler-utils` (bị thiếu, `apt-get update` trước rồi cài lại mới thành công) để dùng Read tool render từng trang, đọc trực quan toàn bộ 72/72 trang (Phần I danh mục, Phần II nội dung chi tiết, Phần III mẫu đơn). Xác nhận lại 10 dòng/5 tên/phân bố cấp đúng như vòng trước. Đọc đầy đủ mục 3 (bãi bỏ) lần đầu: 28 thủ tục cũ có mã thật (6 TW + 8 tỉnh + 14 xã), không phải chỉ 14 xã như biết trước đây. Đọc Phần II đầy đủ cho cả 5 title NEW (T1-T5) và 2 mã AMENDED (1.012564: "Điều chỉnh... theo đề nghị của công dân" — xác nhận khác đối tượng T2/T5; 1.014060: "Tích hợp, cập nhật, điều chỉnh thông tin trên thẻ căn cước" — tên đầy đủ nay đã có). Xác nhận T1/T2 nội dung giống hệt nhau ở cả 3 cấp (chỉ khác cơ quan thực hiện) — Case A theo task. Quyết định catalog scope: CHỈ thêm 5 record cấp XÃ (thay thế đúng loại cho 14 record cũ vừa bị bãi bỏ, cũng chỉ ở cấp xã) — KHÔNG thêm bản Trung ương/tỉnh dù đã có đủ PRIMARY_CONFIRMED content, vì dự án đã chốt quyết định "ưu tiên cấp xã" (docs/brain/03-decisions.md, [2026-07-15]) và catalog can_cuoc trước giờ chỉ có cấp xã — thêm TW/tỉnh sẽ là mở rộng scope ngoài phép của task. Viết đầy đủ nội dung 5 record mới (trình tự/hồ sơ/phí/thời hạn/mẫu đơn/căn cứ pháp lý) theo đúng Phần II, cập nhật nội dung 2 record AMENDED theo nguồn mới nhất. Catalog: 78 → 83.
- **File đã sửa:** `data/tthc-catalog.json` (+5 ADD_NEW, 2 UPDATE_EXISTING, chạy `node scripts/apply-tthc-legal-refresh-to-catalog.js --apply` để đồng bộ categories/legalRefresh), `data/tthc-index.json` (regenerate cùng script), `data/tthc-2026-refresh-manifest.json` (14 record ABOLISHED xã cũ gắn mã thật + successor thật; +14 record ABOLISHED mới cho TW/tỉnh; khối `qd5230_final_catalog_closure` mới), `lib/tthc-legal-refresh.js` (nới lỏng có điều kiện 2 validator check cũ — chỉ cho phép successor/unmapped khi có `qd5230_catalog_closure.evidence_quality === 'PRIMARY_CONFIRMED'` hoặc nhãn "không áp dụng" rõ ràng, không nới vô điều kiện), `test/tthc-legal-refresh.test.js` (viết lại test QĐ5230 cho 28 record + ADD_NEW thật), `docs/tthc/TTHC_2026_QD5230_FINAL_CATALOG_CLOSURE.md` (báo cáo mới).
