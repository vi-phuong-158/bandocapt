...
- **REGRESSION RESULTS:** **CHƯA CHẠY ĐƯỢC** — `scripts/run-regression.js` cần Pinecone + provider key thật (`.env` không có, `PINECONE_API_KEY`/`GEMINI_API_KEY`/`DEEPSEEK_API_KEY` đều unset). Gate H3 **vẫn treo**; chủ dự án phải chạy trong môi trường có credential trước khi merge.
- **CẢNH BÁO về chính bộ regression:** chạy thử `node scripts/run-regression.js --ids TR01` khi thiếu key thì runner **exit 0 và in `Grade: PASS`** dù response rỗng và lỗi là `SERVER_CONFIG_ERROR`. Tức là ở môi trường không có credential, bộ regression cho **false green**. Không được coi nó là cổng đã đạt nếu không xác nhận có key thật. Đây là vấn đề sẵn có của runner, không phải do PR-1, và nên được xử lý riêng.
...
- **MANUAL STEPS:** (1) chạy regression grader **có credential thật** (xem cảnh báo false-green ở trên); (2) smoke-test Vercel Preview trên giao diện chat website thật; (3) owner review diff.
