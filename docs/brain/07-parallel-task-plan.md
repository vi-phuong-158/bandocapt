...
**Cập nhật T3.6–T3.8 (2026-07-17):** 346 law/guide đã duyệt được seed và verify trong namespace ứng viên (tổng 503 vector). Cutover thử ban đầu giảm shadow xuống 73 PASS/18 WARN; đã rollback Production. Sau sửa current-procedure-first, shadow trở lại 88 PASS/2 WARN/0 FAIL và ca lỗi mạng XE03 PASS khi retry; CI 299/299. T3.7 retrieval coi như đạt. T3.8 **CHƯA ĐẠT/CHƯA PHÁT HÀNH** vì majority generation gặp Gemini 429 quota 20 request/ngày và run 1 có hard-fail; Production hiện ở namespace cũ cho tới khi chạy lại strict gate sạch.
