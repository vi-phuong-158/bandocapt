...
- Metadata mới: `cap_normalized`, `canonical_procedure_key`, `submission_channel`, `support_authority`. Hai nguồn hiện hành cùng khóa thủ tục mâu thuẫn fact quan trọng sẽ trả `RAG_CONFLICT`, không gọi model để suy đoán.
  ...
  - Cổng `approved`/`current_procedure`/hiệu lực/cấp trong `buildGovernanceFilter`/`filterGovernedMatches` **chỉ áp dụng cho `source_type='tthc'`** (`requiresProcedureGovernance`). Record `law`/`guide`/thiếu `source_type` luôn bypass — đây là 346 record (152 law + 194 guide) đã tồn tại trước Giai đoạn 3, không mang facts vận hành (phí/thời hạn/biểu mẫu) nên không thuộc phạm vi rủi ro giấy/NA17/hết hiệu lực. `scripts/backfill-law-guide-governance.js` gán tường minh `source_type`/`source_priority` (`legal_basis`/`supplemental`) cho 346 record này, dry-run mặc định, `--apply` cần `PINECONE_API_KEY`.
