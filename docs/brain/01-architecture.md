...
| `scripts/run-regression.js` | Runner regression API that, loc theo ID (ca ID hoi thoai H16/H17); gui `evalDebug:true` va tu cham DU 30 ca bang `lib/regression-grader.js` (2 lop). T1.10: chay hoi thoai nhieu luot tu `test/regression-conversations.json` (truyen `history` tung luot, transcript trong bao cao, cham luot cuoi, thong ke tach rieng); co `--strict-gate` exit 1 khi hard fail HOAC provider error (mac dinh: chi hard fail, gom ca hoi thoai). Global forbidden mac dinh ap dung; fixture cong dan co the opt-out ro rang bang `expectation.use_global_forbidden=false`. T1.11: `--majority`/`--runs N` (mac dinh 3) chay N run day du, ham thuan `aggregateMajority` xep moi ca la HARD FAIL THAT khi rot >= da so (⌊N/2⌋+1) va flaky (advisory) khi rot le te; bao cao tong hop `regression-majority-*.md`. Refactor noi bo: `executeSuiteOnce`/`buildReportMd`/`writeReport` tach chay khoi bao cao | CLI / agent | `api/chat.js`, `lib/regression-grader.js`, `lib/regression-metrics.js`, `test/regression-expectations.json`, `test/regression-conversations.json`, `test/cau-hoi/bo-test-regression-30-cau-nguoi-nuoc-ngoai-tthc.md`, `test/results/` |
