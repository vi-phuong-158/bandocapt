...
- **Sửa:** thêm `LocationTaxonomy.toCanonicalSiteType(value)` (mirror `toCanonicalServices`, trả về mã canonical hoặc `null`) vào `lib/location-taxonomy.js`. `app.js` gom toàn bộ phân loại vào 2 hàm dùng chung `isPoliceLocation(loc)`/`isCccdLocation(loc)` (cache canonical qua `canonicalSiteType(loc)`/`canonicalServiceCodes(loc)` trên `loc._canonicalSiteType`/`loc._canonicalServiceCodes`): ưu tiên `siteType`/`services` canonical, chỉ fallback về biểu thức legacy cũ khi bản ghi chưa có `siteType` đọc được. 6 nơi từng lặp lại biểu thức inline (marker icon, mobile preview, detail badge + combo-badge + procedure note, quick filter, result list) nay gọi chung 2 hàm này — không còn lệch pha giữa các bề mặt.
- **Tương thích ngược:** bản ghi không có `siteType` (đa số dữ liệu hiện tại) đi đúng nhánh fallback, giữ nguyên hành vi cũ 100% — đã xác minh bằng cách tạm revert bản vá và chạy lại đúng bộ test mới, thấy test canonical fail còn test legacy vẫn pass.
- **Test:** `test/location-taxonomy.test.js` (contract `toCanonicalSiteType`), `test/location-ui.test.js` (characterization: 2 hàm tồn tại, canonical-first, biểu thức fallback chỉ còn đúng 1 lần mỗi loại), `test/e2e/location-taxonomy-filter.spec.js` (browser thật: `PUBLIC_SERVICE_CENTER + [IDENTITY]` phải hiện dưới filter "Điểm CCCD", biến mất khi tắt filter đó dù "Công an" đang bật, marker/detail badge đúng; kèm 1 case legacy police + 1 case legacy CCCD để chứng minh không regression).
- **Không đổi:** UI (0 thay đổi pixel/markup), `app.js:185–340` (sheet state machine), chatbot, Nhà trọ Beta, schema/API.
...
| `lib/response-sink.js` | Ranh gioi transport cua `/api/chat`: `createSseSink(res)` tai tao DUNG tung byte SSE hien tai (`writeHead`/`write`/`end`/`status().json()`), `createBufferSink()` nhan cung chuoi su kien trong bo nho khong can HTTP. Giu luon `startSseHeartbeat` vi heartbeat phu thuoc vong doi stream Node (`writableEnded`/`close`/`finish`) — thuan transport. Sink KHONG dinh dang, KHONG dien giai noi dung. | `api/chat.js`, test | Node stream lifecycle only |
