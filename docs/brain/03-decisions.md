...
- **Quyet dinh:**
  ...
  4. **Groundedness check (canh bao, khong chan)** — them `checkGroundednessAsync()`, chay fire-and-forget SAU `res.end()` (khong tang latency nguoi dung thay). Neu answer chua so lieu co don vi, goi Gemini Flash doi chieu voi legalCorpus, ghi ket qua vao Firebase `groundedness_checks/<date_key>`. Day la lop giam sat THEM, khong thay the `lib/output-validator.js` (van fail-closed nhu cu).
  ...
  7. **Hieu nang:** `reserveRateLimitQuota` doi tu tuan tu (IP/ngay roi thang) sang **song song** qua `Promise.all` — rollback ben da reserve thanh cong neu ben kia fail (logic rollback giu nguyen, chi doi thu tu orchestration). DeepSeek timeout 50s xac nhan hop le vi `vercel.json` co `maxDuration: 60` (chi them comment).
