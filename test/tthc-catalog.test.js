...
test('data/tthc-catalog.json đã commit: chỉ thủ tục TTHC thật, sạch guide và trùng lặp', () => {
    const catalogPath = path.resolve(__dirname, '..', 'data', 'tthc-catalog.json');
    const catalog = JSON.parse(fs.readFileSync(catalogPath, 'utf8'));

    assert.equal(catalog.sourceMode, 'live');
    assert.equal(catalog.includeGuides, false, 'catalog commit phải tắt guide (chỉ thủ tục thật)');
    // Bộ TTHC thật ~35; chặn cả trường hợp thiếu và trường hợp guide lọt lại (100+).
    assert.ok(
        catalog.procedures.length >= 30 && catalog.procedures.length <= 60,
        `Kỳ vọng ~35 thủ tục TTHC thật, có ${catalog.procedures.length}`
    );

    // Không được lẫn entry guide (wiki/FAQ/hướng dẫn nội bộ chatbot)
    assert.ok(
        catalog.procedures.every(p => !String(p.procedureId).startsWith('guide:')),
        'catalog không được chứa entry guide'
    );
