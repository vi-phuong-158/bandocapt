...
// R2a canonical panel-state contract: exactly one of three mutually-exclusive UI surfaces owns
// the screen at a time — browsing (search-panel reachable), detail (a location's sheet is open),
// or mobile-search (the mobile-only search overlay). `applyPanelChrome` in app.js is the sole
// writer of this state; see docs/brain/03-decisions.md and docs/redesign/CLAUDE_REVIEW_R0_V1.md
// (ARCH-1) for context, and the R1 arbiter spec (location-visibility-arbiter.spec.js) for the
// marker-layer half of this invariant.
//
...
test.describe('desktop panel reachability', () => {
    ...
    test('quick filters are unreachable while detail is open, and reachable again through the canonical close control', async ({ page }) => {
        await page.goto('/');

        const policeLabel = page.locator('label:has(#filter-police)');
        // Baseline: before any location is selected, the filter is a real, clickable target.
        await expect(policeLabel).toBeVisible();

        // Fixture index 1 ("khu vực 2") is police-typed — see scripts/preview-server.js.
        ...
        // detail-panel stacked above at a higher z-index). A real click at the filter label's own
        // coordinates must not reach it — Playwright's actionability check enforces this the same
        // way a real mouse would. This is the bug R1 flagged and R2a is scoped to resolve.
        await expect(policeLabel.locator('input')).not.toBeChecked({ timeout: 1000 }).catch(() => {});
        await expect(async () => {
            await policeLabel.click({ timeout: 800 });
        }).rejects.toThrow();
        ...
        // Filters are now genuinely reachable by a real click, and the click has a real effect.
        await policeLabel.click();
        await expect(page.locator('#filter-police')).not.toBeChecked();
