...
test('buildCitizenSummary không hiển thị tên file nguồn (.docx) ở ô "Nộp tại"', () => {
    ...
    // Guide thiếu "Cơ quan xử lý" và chỉ có "Nguồn" là tên file → ô "Nộp tại"
    // phải trả về câu trung tính, không lộ tên file nội bộ cho người dân.
    assert.deepEqual(
    ...
    assert.ok(summary.every(item => !/\.docx/i.test(item.value)));
});
